...
export default function Swap() {
  ...
  const { data: currentGreeting, isLoading: isGreetingLoading } = useScaffoldContractRead({
    contractName: "ZuniswapV2Library",
    functionName: "getAmountsOut",
    args: [tokenAddr0, parseEther(tokenAmount0), [tokenAddr0, tokenAddr1]],
  });

  const { writeAsync, isLoading } = useScaffoldContractWrite({
    contractName: "ZuniswapV2Router",
    functionName: "swapExactTokensForTokens",
    args: [parseEther(tokenAmount0), parseEther(tokenAmount1), [tokenAddr0, tokenAddr1], address],
