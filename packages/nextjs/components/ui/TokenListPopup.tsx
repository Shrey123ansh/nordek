...
const TokenListPopup: React.FC<TokenListPopupProps> = ({ isOpen, onClose, setToken }) => {
      ...
      <div className="max-h-96 flex-grow overflow-y-auto bg-base-300">
        {Object.values(tokens).map((token, index) => (
          <div
            ...
            onClick={() => { setToken(token); onCloseOverride() }}
          >
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center z-50">
      ...
      <div className="relative z-10 bg-base-300  rounded-lg shadow-lg p-6 max-h-[500px] min-w-[550px] ">

        <div className="flex  flex-col " >

          {/* header part  */}
          <div className="flex flex-row justify-between  items-center  " >
            <div className="text-lg text-center font-bold text-white">{heading}</div>
            <button className="absolute top-0 right-0 m-2 p-2 text-gray-400 hover:text-primary-content" onClick={onCloseOverride}  >
              <svg
...

          {
            heading === heading2 && <>
              <div className="flex  w-full " >
                <input type="text" className=" my-4  py-2 w-full px-2 " placeholder="Search by Address" value={searchValue} onChange={(event) => {
              ...
              {loadingToken && <div className="text-white font-bold mt-4">Loading...</div>}
              {
                newToken.name !== "" && newToken.symbol !== "" && <div
                  ...
                  {newToken.status === "not added" && <button className="ml-20" onClick={addNewTokenToList} >+ Add</button>}
                  {newToken.status === "added" && <button className="ml-20" onClick={removeToken} >- Remove</button>}
                </div>
              }
            </>
          ...
          {/* token list part  */}
          {heading == heading1 && <div className=" flex flex-col max-h-[300px]     overflow-y-scroll " >
            <ListComponent />
          </div>
          }
          <div>
            <div className="   text-left mt-8   text-white font-bold text-[15px] "    >
              <button onClick={onFotterClick} >
                {fotter}
              </button>
            </div>
          </div>
