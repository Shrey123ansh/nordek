...
const trades = [
  { usdValue: "$100", currency1: {amount: "0.5", token: "ETH"}, currency2: {amount: "150", token: "USDT"}, time: Date.now() - 120000 },
  { usdValue: "$100", currency1: {amount: "0.5", token: "ETH"}, currency2: {amount: "150", token: "USDT"}, time: Date.now() - 120000 },
  { usdValue: "$100", currency1: {amount: "0.5", token: "ETH"}, currency2: {amount: "150", token: "USDT"}, time: Date.now() - 120000 },
  { usdValue: "$100", currency1: {amount: "0.5", token: "ETH"}, currency2: {amount: "150", token: "USDT"}, time: Date.now() - 120000 },
  { usdValue: "$100", currency1: {amount: "0.5", token: "ETH"}, currency2: {amount: "150", token: "USDT"}, time: Date.now() - 120000 },
  ...
  { usdValue: "$100", currency1: {amount: "0.5", token: "ETH"}, currency2: {amount: "150", token: "USDT"}, time: Date.now() - 120000 },
     ...
     function TradeHistory({ trades }) {
      return (
        <div className="border-b-2 mb-4 bg-purple-100">
    <h2 className="text-xl font-bold p-2 text-purple-600">Trade History</h2>
    <div className="max-h-60 overflow-y-auto scrollbar-hide">
        <table className="min-w-full divide-y divide-gray-200">
            <thead className="bg-purple-200">
                <tr>
                    <th className="px-6 py-3 text-left text-xs font-medium text-purple-600 uppercase tracking-wider">USD Value</th>
                    <th className="px-6 py-3 text-left text-xs font-medium text-purple-600 uppercase tracking-wider">Amount</th>
            ...
            {/* @ts-ignore */}
                {trades.map((trade, idx) => (
                    <React.Fragment key={idx}>
                        <tr>
                            <td rowSpan={2} className="px-6 py-4 whitespace-nowrap text-purple-600">{trade.usdValue}</td>
                            <td className="px-6 py-2 whitespace-nowrap text-purple-600">{trade.currency1.amount} {trade.currency1.token}</td>
                            <td rowSpan={2} className="px-6 py-4 whitespace-nowrap text-purple-600">{formatTime(trade.time)}</td>
                        </tr>
                        <tr>
                            <td className="px-6 py-2 whitespace-nowrap text-purple-600">{trade.currency2.amount} {trade.currency2.token}</td>
                        </tr>
                    </React.Fragment>
                ))}
            </tbody>
