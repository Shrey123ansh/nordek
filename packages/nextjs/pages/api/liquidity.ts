...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  const collectionName = "userLiquidities";
  ...
  switch (req.method) {
    case "POST":
      try {
        ...
        const existingLiquidity: Liquidity = await db
          .collection(collectionName)
          .findOne({ pairContract: liquidity.pairContract, user: liquidity.user });
    ...
    case "GET":
      ...
      const userLiquidityValues = await db
        .collection(collectionName)
        .find({ user: address as string })
        .toArray();
    ...
    case "DELETE":
      try {
        ...
        const result = await db
          .collection(collectionName)
          .deleteOne({ user: userAddress as string, pairContract: pairContract as string });
