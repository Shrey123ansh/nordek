    ...
    switch (req.method) {
        case "POST":
            try {
                ...
                console.log("USER LIQUIDITY DATA ADDED");
        ...
        case "GET":
            ...
            const userLiquidityValues = await db.collection(collectionName).find({ address }).sort({ symbol: -1 }).toArray();
        ...
        case "PUT":
            try {
                ...
                const result = await db.collection("userLiquidity").updateOne(filter, updateDoc);
